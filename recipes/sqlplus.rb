...
basic_file_name = "instantclient-sqlplus-linux#{arch_flag}-#{node[:oracle_instantclient][:client_version]}.zip"
...
remote_file "#{install_dir}/dist/#{basic_file_name}" do
  ...
  notifies :run, "execute[unpack_instant_client_sqlplus]", :immediately
end

execute "unpack_instant_client_sqlplus" do
